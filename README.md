# 📚 Comic Reader App

A web-based comic reader built with **React**, **React Router**, and **Material-UI**. This app allows users to browse comic details, view chapters, and read each chapter page-by-page with a clean and responsive layout.

---

## 🔧 Features

- 📖 Browse multiple comics
- 🎯 Navigate through chapters easily
- 🌟 Trending and tag-based organization
- 🔄 Fully responsive design using Material UI

---

## 🛠️ Installation & Setup

To run this project locally:

### 1. Clone the repository

git clone https://github.com/Vantom0v0/midterm_project.git

cd midterm_project

### 2. Install dependencies

npm install (or npm i)

npm install @mui/material @emotion/react @emotion/styled @mui/icons-material

npm audit fix (if there are any vulnerabilities)

### 3. Start the development server

npm run dev

The app will be available at http://localhost:3000

## 📁 Project Structure
<pre>
  src/
  ├── assets/
  │   └── comicsData.js           # Comic metadata
  │   └── pagesPerChapter.json    # Page count for each chapter
  ├── components/
  │   └── ComicPageNavbar.jsx
  │   └── Trending.jsx
  │   └── Footer.jsx
  ├── pages/
  │   └── homepage.jsx
  │   └── comicPage.jsx
  │   └── chapterPage.jsx
  ├── App.jsx
  └── index.js
</pre>

## 📦 Dependencies
React

React Router

Material-UI

@mui/material

@mui/icons-material

@emotion/react

@emotion/styled

## 👥 Contributors
This project was developed by the following students as part of their coursework:

- Vantom0v0 (Student ID: 523K0039)

- Haune (Student ID: 523K0038)

- Huin (Student ID: 523K0041)
